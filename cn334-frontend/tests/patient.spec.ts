...
test('ทดสอบระบบค้นหาคนไข้บนตาราง (ด้วย API Mocking)', async ({ page }) => {
  // 1. ดักจับและปลอมแปลงขอมูล API (Mocking)
  // ทันทีที่หนnpmาเว็บพยายามจะวิ่งไปหา localhost:3340/patients Playwright จะดักไวและสงขอมูลปลอมนี้กลับไปแทน
  await page.route(`${process.env.NEXT_PUBLIC_API_URL}patients`, async route => {
    const mockData = [
  ...
  // 2. ไปที่หนาจัดการคนไข
  await page.goto('http://localhost:3000/admin/patients');
  // 3. ตรวจสอบวาตารางโหลดขอมูลปลอมของเรามาแสดงผลเรียบรอยแลว
  // expect จะรอจนกวาคําวา Robot Tester จะปรากฏ (หมดปญหาเรื่องโหลดขอมูลไมทัน)
  await expect(page.locator('table')).toContainText('Robot Tester');
  // 4. ทดสอบระบบคนหา: พิมพคนหาคําวา "John"
  const searchInput = page.getByPlaceholder('ค้นหาชื่อ หรือ HN...');
  await searchInput.fill('John');
  // 5. ตรวจสอบผลลัพธ: ตารางตองแสดง John Doe และตอง "ไม" แสดง Robot Tester แลว
